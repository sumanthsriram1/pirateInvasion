...
function draw() {
  ...
  if(!backgroundMusic.isPlaying()){
    ...
    backgroundMusic.setVolume(.1) // 100% --> 1 , 50% --> 0.5
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        if(collision.collided && !boats[i].isBroken){ 
           isGameOver = true;
           gameOver()
           if(!isPirateLaughing && !pirateLaugh.isPlaying()){
            pirateLaugh.play()
            pirateLaugh.setVolume(.1);
...
function gameOver(){
  swal ({
    ...
    imageUrl:"https://i.gifer.com/AN53.gif",
    imageSize:"250x350",
